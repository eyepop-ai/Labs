...
async def main():
    ...
    # print(asset)
    ability_info = fetch_ability_info(asset.get("dataset_uuid"), source_of_interest, token)
    ...
    false_positives, false_negatives = find_false_positives_negatives(predictions, ground_truth)

    # send false positives to vlm for description
    fp_desc, fn_desc = await process_mismatch_samples_and_collect_results(
        asset_uuid=asset_uuid,
        ACCOUNT_UUID=ACCOUNT_UUID,
        api_key=api_key,
        token=token,
    ...
    refinement_prompt = build_refine_prompt(fp_desc, fn_desc, og_prompt, timestamp)
    print("Refinement Prompt:")
    print(refinement_prompt)

    better_prompt = generate_prompt_with_chatgpt(refinement_prompt, timestamp)
