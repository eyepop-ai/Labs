...
for folder in sub_folders:
    ...
    for image in images:
        ...
        prompt = f"Describe only the visual features that would help distinguish this image from images belonging to other vehicle-related categories. Avoid mentioning filenames, paths, or class names. Focus on what is visible and unambiguous."
        result = ""
        ...
        if 'raw_output' in result:
            ...
            results.setdefault(folder, []).append(result)
            print(f"Result for image {image} in folder {folder}: {result}\n\n")
        else:
            print(f"No raw_output found for image {image} in folder {folder}\n\n")        


creation_prompt = "In the following, you will find a series of image descriptions in xml tags. The xml tag defines the class of the image. Your task is to create a single comprehensive prompt that will allow a VLM to classify images into their respective classes based on the descriptions provided. Ensure that the prompt is clear, concise, and covers all the necessary details to facilitate accurate classification by the VLM.\n"
...
creation_prompt += f"\n\nThe possible output labels are: {class_labels}. Do not include explanations, confidence scores, or additional text. Choose the closest matching category based on the dominant visual content."
